...
import { buy, PortfolioServiceError } from '@/src/services/portfolioService'
...
export default function BuyPage() {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    setLoading(true)
    try {
      const data = await buy(ticker.trim().toUpperCase(), qty)
      setSuccessMsg(`Compra registrada: ${data.quantity} unidades de ${data.ticker}`)
      setQuantity('')
    } catch (error) {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 px-4 pt-24 pb-8">
      <div className="max-w-sm mx-auto space-y-8">
        <div className="space-y-4">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Buy shares</h1>
        </div>
